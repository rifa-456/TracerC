...
/**
 * @brief Lê um buffer de um tamanho específico da memória de um processo rastreado.
 * @param pid O PID do processo alvo.
 * @param addr O endereço de memória do buffer no processo alvo.
 * @param count O número de bytes a serem lidos.
 * @return Uma representação do buffer em string, com caracteres não imprimíveis escapados.
 */
static std::string read_buffer_from_process(pid_t pid, unsigned long addr, size_t count)
{
    if (addr == 0)
        return "NULL";
    std::string out;
    ...
    while (bytes_read < count)
    {
        ...
        char *bytes = reinterpret_cast<char *>(&word);
        for (size_t i = 0; i < sizeof(long) && bytes_read < count; ++i)
        {
            // Adiciona caracteres imprimíveis ou sequências de escape
            char c = bytes[i];
            if (isprint(c) || c == '\n' || c == '\t' || c == '\r')
            {
                out += c;
            ...
            {
                out += fmt::format("\\x{:02x}", (unsigned char)c);
            }
            bytes_read++;
        }
    }
...
{
    // Se o tipo for um ponteiro de caractere, trata-o como uma string e lê da memória do processo.
...
void fork_and_trace(const std::vector<std::string> &args)
{
    if (args.empty())
    {
        spdlog::error("Nenhum programa especificado para o fork.");
        return;
    }
    ...
    if (child < 0)
    {
        spdlog::critical("fork falhou: {}", strerror(errno));
        return;
    }
...
void Tracer::run()
{
    ...
    while (!m_threads_in_syscall.empty())
    {
        ...
        {
            spdlog::debug("[run] pulando pid não rastreado={}", pid);
        ...
        if (sig == (SIGTRAP | 0x80))
        {
            ...
            else
            {
                ...
                {
                    spdlog::info("SYSCALL_EXIT  [PID:{}] execve bem-sucedido", pid);
                    m_just_execed[pid] = false;
                }
                else
                {
    ...
    spdlog::info("[run] saindo do loop principal");
}

/**
 * @brief Registra os detalhes de uma chamada de sistema em seu ponto de entrada.
 * @param pid O PID do processo que está fazendo a chamada.
 */
void Tracer::log_syscall_entry(pid_t pid)
{
    ...
    {
        spdlog::warn("[entry] GETREGS falhou para {}: {}", pid, strerror(errno));
        return;
    }
    ...
    {
        std::string args_str;
        long long vals[] = {(long long)regs.rdi, (long long)regs.rsi,
                            (long long)regs.rdx, (long long)regs.r10,
                            (long long)regs.r8,  (long long)regs.r9}; // Os 6 registratores de falor

        // Tratamento especial para write
        if (info->name == "write" && info->arg_count >= 3)
        {
            // write(fd, buf, count)
            // fd: regs.rdi (vals[0])
            // buf: regs.rsi (vals[1])
            // count: regs.rdx (vals[2])
            std::string fd = fmt::format("{}", vals[0]);
            std::string buf = read_buffer_from_process(pid, vals[1], vals[2]);
            std::string count = fmt::format("{}", vals[2]);
            args_str = fmt::format("{}, {}, {}", fd, buf, count);
        }
        else // Lógica genérica para todas as outras syscalls
        {
            for (int i = 0; i < info->arg_count; ++i)
            {
                if (i > 0)
                    args_str += ", ";
                const std::string &type = info->arg_types[i];
                long long value = vals[i];

                if (type.find("char") != std::string::npos &&
                    type.find('*') != std::string::npos) // Condicional que procura 'char' E '*'
                                                         // dentro da string "type"
                {
                    args_str += read_string_from_process(pid, value);
                }
                // Formata números grandes como hex
                else if (value > 1000000)
                {
                    args_str += fmt::format("{:#x}", value);
                }
                // Formata como decimal simples
                else
                {
                    args_str += fmt::format("{}", value);
                }
            }
        }
    ...
    {
        spdlog::warn("SYSCALL_ENTRY [PID:{}] desconhecida {}", pid, regs.orig_rax);
    }
}
...
/**
 * @brief Desanexa de todos os processos rastreados.
 *
 * É chamado durante a limpeza para garantir que os processos rastreados possam
 * continuar a execução normalmente após o tracer sair.
 */
void Tracer::shutdown()
{
    // Loop que vai percorrer procurar todos pids
    for (const auto &[pid, in_syscall] : m_threads_in_syscall)
    {
        spdlog::info("Desanexando do processo {}", pid);
        if (ptrace(PTRACE_DETACH, pid, nullptr, nullptr) ==
            -1) // PTRACE_DETACH para de rastrear o processo atual e permite que ele continue.
        {
            spdlog::error("Falha ao se desanexar do processo {}: {}", pid, strerror(errno));
        }
    }
    m_threads_in_syscall.clear();
}