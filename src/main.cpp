...
#include <sys/wait.h>
#include <vector>
static Tracer *g_tracer_ptr = nullptr;
...
/**
 * @brief Handles termination signals for a clean shutdown.
 * @param sig The received signal number.
 *
 * Invokes the tracer's shutdown() method to detach from all tracees.
 */
static void cleanup_handler(int sig)
{
    spdlog::warn("Sinal {} recebido, desligando  tracer.", sig);
    if (g_tracer_ptr != nullptr)
    {
        g_tracer_ptr->shutdown();
    }
}
...
int main(int argc, char *argv[])
{
    ...
    {
        // Bloco de código para fazer a função "cleanup_handler" ser chamada quando SIGINT ou
        // SIGTERM for recebido
        ...
        for (pid_t pid : pids_to_trace)
        {
            ...
            ptrace(PTRACE_SETOPTIONS, pid, nullptr,
                   PTRACE_O_TRACESYSGOOD | PTRACE_O_TRACECLONE | PTRACE_O_TRACEFORK |
                       ...
                       PTRACE_O_EXITKILL); // Setar as configurações do ptrace, basicamente fazendo
        ...
        g_tracer_ptr = &tracer; // Ponteiro para o tracer para ser possivel desligar ele sem afetar
                                // o processo anexado
